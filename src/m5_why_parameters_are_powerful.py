...
def main():
    ...
    better_draw_circles(rg.Point(0,0),3)
    better_draw_circles(rg.Point(200, 30), 10)

    even_better_draw_circles(rg.Point(-100,80),5,20,'pink',1)
    window.update()
...
def better_draw_circles(point,increaseinradius):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(increaseinradius)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(increaseinradius * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point,increaseinradius,numberofcircles,pencolor,penthickness):

    turtle = rg.SimpleTurtle()
    turtle.pen = rg.Pen(pencolor,penthickness)
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(1, numberofcircles):  # k becomes 1, 2, 3, ... 10
        ...
        turtle.pen_down()
        turtle.draw_circle(increaseinradius * k)
